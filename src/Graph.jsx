...
import metroImage from "./data/ImageStations/amsterdam_metro.jpg"
// import d3 from 'd3'
const d3 = window.d3
...
//imports for data already processed
let novemberData = require('./data/NovFormattedData_normalized.json')
let decemberData = require('./data/DecFormattedData_normalized.json')
let suppliedData;
...
class Graph extends React.Component {
    ...
    componentDidMount() {
        ...
        var noOfArrivals = 'AantalReizen';
        var arrivalTime = 'UurgroepOmschrijving (van aankomst)';
        var stationName = 'AankomstHalteNaam';
        var scaleSegmentRatio = 18;

        var currentDate = moment('12/06/2021', 'M/D/YYYY');

        (function changeDate(){
            let windowPath = window.location.pathname.substring(1);
            currentDate = moment(windowPath, 'MM-DD-YYYY');
            if(currentDate.month() + 1 === 12) {
                suppliedData = decemberData;
            }
            if(currentDate.month() + 1 === 11)
                suppliedData = novemberData;
        }());

        const oldDate = __.cloneDeep(currentDate);

        console.log("Current working date is " + currentDate.format('M/D/YYYY'));

        //resNovemberData already sorted by date and time
        ...
        //sorts the amsterdam_delay_2.json file by days
        ...
        var byDay2 = _.chain(suppliedData)
            .filter(function (d){
                let lookingAtDate = moment(d['Datum'], 'M/D/YYYY');
                let daysDiff = moment.duration(lookingAtDate.diff(currentDate));
                return daysDiff.asDays() >= 0 && daysDiff.asDays() < 7;
            })
            .toArray()
            .value();

        for(let i = 0; i < byDay2.length; i++)
        {
            for(let j = 0; j < byDay2[i]['Details'].length; j++)
            {
                let entries = byDay2[i]['Details'][j];
                let arrTimeSplit = entries[arrivalTime].split(' - ');
                let timeOfEntry = arrTimeSplit[0].split(':');
                //attempt to preserve code style & structure
                let secOfDay = 60 * 60 * parseFloat(timeOfEntry[0]);
                byDay2[i]['Details'][j][arrivalTime] = secOfDay;

                //set day - to integers
                byDay2[i]['Details'][j].day = i;
            }
        }
        ...
        dayRowLabels.append('text')
          ...
          .text(function (d) {
              let displayDate = currentDate.add(d, 'days');
              currentDate = __.cloneDeep(oldDate);
              return moment.weekdaysShort()[displayDate.weekday()];
          });
        ...
        dayRowLabels.append('text')
          ...
          .text(function (d) {
              let displayDate = currentDate.add(d, 'days');
              currentDate = __.cloneDeep(oldDate);
              return displayDate.format('MMM') + ' ' + displayDate.format('DD');
          });
        ...
        horizons.selectAll('.horizon')
            .data(function (d) {
                ...
                // var min_old = d3.min(delay, function (t) {
                //     return t[horizonType];
                // });
                // var max_old = d3.max(delay, function (t) {
                //     return t[horizonType];
                // });

                // var scale_old = d3.scale.linear().domain([min_old, max_old]);
                ...
                //         .where({day: d})
                //         .filter(function (t) {
                //             return typeof (t[horizonType]) === 'number';
                //         })
                //         .map(function (t) {
                //             return [t.secOfDay, scale_old(t[horizonType])];
                //         })
                //         .value()
                // ];


                var min = d3.min(byDay2, function (t) {
                    let mn = Infinity;
                    let details = t['Details'];
                    let mn_aux = d3.min(details, function (x){
                        return d3.min(x['ins'], function(m){
                            return parseFloat(m[noOfArrivals]);
                        });
                    });
                    if(mn > mn_aux)
                        mn = mn_aux;
                ...
                var max = d3.max(byDay2, function (t) {
                    let mx = 0;
                    let details = t['Details'];
                    let mx_aux = d3.max(details, function (x){
                        return d3.max(x['ins'], function(m){
                            return parseFloat(m[noOfArrivals]);
                        });
                    });
                    if(mx < mx_aux)
                ...
                var result = [
                    _.chain(byDay2[d]['Details'])
                        .map(function (t) {
                            return [t[arrivalTime], scale(parseFloat(t[horizonType]))];
                        })
            ...
            .enter()
          .append('g')
            .attr('class', function (d) { return 'horizon dimmable day-' + d; })
            .call(horizon);
        ...
        // datapoints
        //TODO - match the gradiant band (when we have new delay info)
        var gradient = chart.append("svg:defs").selectAll('linearGradient')
            .data(byDay2)
            .enter()
          .append("svg:linearGradient")
            .attr("id", function (d) { return "gradient" + d['Details'][0].day; })
            .attr("x1", "0%")
            .attr("y1", "0%")
            .attr("x2", "100%")
            .attr("y2", "0%")
        ...
        gradient.selectAll('stop')
          ...
          .append("svg:stop")
            ...
            .attr("offset", function (d) {
                //return delayMapXScale(d.secOfDay + 60 * 7.5);
                return delayMapXScale( d[arrivalTime] + 60 * 7.5);
            })
            .attr("stop-color", function (d) {
                //TODO: Look more into the normalization step, do it beforehand in python!
                return delayMapColorScale(d['delay_actual']);
            })
            .attr("stop-opacity", 1);
        ...
        var bisect_old =  d3.bisector(function (d) { return d.secOfDay; }).left;
        ...
        function mouseover() {
          ...
          var day = Math.max(0, d3.bisectLeft(dayRowYScale.range(), y) - 1) % 7;
        ...
        function mouseoverTime(day, theTime) {
          ...
          let displayDate = currentDate.add(day, 'days');
          currentDate = __.cloneDeep(oldDate);
          var fullTime = moment(theTime).utc().format('h:mm a') + ' on ' + moment.weekdaysShort()[displayDate.weekday()] + ' ' +
              displayDate.format('MMM') + ' ' + displayDate.format('DD');


          timeDisplayBelowMapGlyph.text(fullTime);
          d3.select('.interaction-all .time').text(fullTime);
          var inputData = byDay2[day]['Details'];
          delays = {};
          var idx = bisect(inputData, (theTime / 1000) ) - 1;
          var ratio = ((theTime-1) % bucketSize) / bucketSize + 0.01;

          var before = inputData[idx] || inputData[idx+1];
          var after = inputData[idx+1] || before;

            // var inputData_old = byDay[day];
            // var idx_old = bisect_old(inputData_old, theTime / 1000 - 15 * 60) - 1;
            // var ratio_old = ((theTime-1) % bucketSize) / bucketSize;
            // var before_old = inputData_old[idx_old] || inputData_old[idx_old+1];
            // var after_old = inputData_old[idx_old+1] || before_old;


            //calculates no of entrances on the given average
            //feeds it to scale the bars
            //before.ins & after.ins: the array of all stations where we map station name to entry no per sec
            let stationToEntryArray1 = {};
            for(let i = 0; i < before['ins'].length; i++)
            {
                let stationName_key = before['ins'][i][stationName];
                let entry_value = before['ins'][i]['deviated_in'];
                if(stationToEntryArray1.hasOwnProperty(stationName_key))
                    stationToEntryArray1[stationName_key] += entry_value;
                else
                    stationToEntryArray1[stationName_key] = entry_value;
                // let pair = [ before['ins'][i][stationName], before['ins'][i]['deviated_in'] ];
                // stationToEntryArray1.push(pair);
            }

            let stationToEntryArray2 = {};
            for(let i = 0; i < after['ins'].length; i++)
            {
                let stationName_key = after['ins'][i][stationName];
                let entry_value = after['ins'][i]['deviated_in'];
                if(stationToEntryArray2.hasOwnProperty(stationName_key))
                    stationToEntryArray2[stationName_key] += entry_value;
                else
                    stationToEntryArray2[stationName_key] = entry_value;
                // let pair = [ before['ins'][i][stationName], before['ins'][i]['deviated_in'] ];
                // stationToEntryArray1.push(pair);
            ...
            let standardizedDelays = {};
            let standardizedDelays1 = {};

            for(let i = 0; i < before['ins'].length; i++)
            {
                let stationName_key = before['ins'][i][stationName];
                let entry_value = before['ins'][i]['standardized_delay'];
                if(standardizedDelays1.hasOwnProperty(stationName_key))
                    standardizedDelays1[stationName_key] += entry_value;
                else
                    standardizedDelays1[stationName_key] = entry_value;
            }

            let standardizedDelays2 = {};
            for(let i = 0; i < after['ins'].length; i++)
            {
                let stationName_key = after['ins'][i][stationName];
                let entry_value = after['ins'][i]['standardized_delay'];
                if(standardizedDelays2.hasOwnProperty(stationName_key))
                    standardizedDelays2[stationName_key] += entry_value;
                else
                    standardizedDelays2[stationName_key] = entry_value;
            }

            function manual_interpolation(vector_a, vector_b, ratio)
            {
                let res = [];
                for(let station_name_key in vector_a)
                {
                    //check if it exists in the other vector
                    if(vector_b[station_name_key] !== 'undefined'){
                        let entry_interpolated = d3.interpolate(vector_a[station_name_key], vector_b[station_name_key])(ratio);

                        //remove from source arrays
                        delete vector_a[station_name_key]
                        delete vector_b[station_name_key]

                        //add to result
                        res[station_name_key] = entry_interpolated;
                    }
                    else
                    {
                        //station name not in b
                        vector_a.splice(station_name_key, 1);
                    }
                }

                for(let station_name_key in vector_b)
                {
                    let entry_interpolated = vector_b[station_name_key] * ratio;
                    delete vector_b[station_name_key]
                    res[station_name_key] = entry_interpolated;
                }
                return res;
             ...
             //entrances = d3.interpolate(stationToEntryArray1, stationToEntryArray2)(ratio);
            entrances = manual_interpolation(stationToEntryArray1, stationToEntryArray2, ratio);

            //standardizedDelays = standardizedDelays1;
            //  standardizedDelays = d3.interpolate(standardizedDelays1, standardizedDelays2)(ratio);
            standardizedDelays = manual_interpolation(standardizedDelays1, standardizedDelays2, ratio);


          turnstileEntryText.text("approx. " + d3.format('0f')(d3.interpolate(before[horizonType], after[horizonType])(ratio)) + " entries/hour");

          //TODO - calculate relative delay & introduce actual delay info

          // var delay = before_old.delay_actual * (ratio_old);
          ...
          // var trainDataLeft = _.extend.apply(null, [{}].concat(_.pluck(before_old.lines, 'delay_actual')));
          // var trainDataRight = _.extend.apply(null, [{}].concat(_.pluck(after_old.lines, 'delay_actual')));
          // var trainDataMerged = d3.interpolate(trainDataLeft, trainDataRight)(ratio_old);
          ...
          function updateCachedDelayForSegment(FROM, TO) {
            var key = FROM + "|" + TO;
              ...
              let to_aux = 0.01;
              let val;
              from_aux = standardizedDelays[FROM] !== 'undefined' ? standardizedDelays[FROM] : 0.01;
              to_aux = standardizedDelays[TO] !== 'undefined' ? standardizedDelays[TO] : 0.01;

              from_aux *= 9;
              to_aux *= 9;

              if(from_aux !== 0.01 && to_aux !== 0.01) {
                  val = from_aux > to_aux ? from_aux : to_aux;
              }else if(from_aux === 0.01 && to_aux !== 0.01){
                  val = to_aux;
              }else val = from_aux;
        ...
        var glyphOuterHeight = 450;
        var glyphOuterWidth = 450;
        ...
        function mapGlyphSegmentColor(d) {
          ...
          var speed = delays[d.ids] !== 'undefined' ? delays[d.ids] : 0.001;
          var color;
          if(isNaN(speed))
              speed = 0;
          if (speed === null || typeof speed === 'undefined') {
            ...
            color = redGreenDelayColorScale(clamp(speed,-10,50));
          }
          return color;
        }
        network.nodes.forEach(function (data) {
        ...
        drawMultiDot('isolatorweg', ["#00FF00","#FFA500"], 'horizontal');
        ...
        (function drawHorizonKey() {
          ...
          var max = d3.max(suppliedData, function (t) {
              return d3.max(t['Details'], function(x){
                  return x[horizonType];
              });
          });
          var xScale = d3.scale.linear()
        ...
        var initalTime = moment('2/3/2014 5:30 PM ', 'M/D/YYYY hh:mm a');

        mouseoverTime(0, initalTime.diff(initalTime.clone().startOf('day')));
        ...
        //used to reduce the size of the segments that can grow out of proportion
        ...
        function offsetPoints(link) {
          var split = link.ids.split("|").map(function (a) {
            var val;
            //TODO: Introduce stop codes and filter for correct direction
            let aux = 0;
            // for(let i = 0; i < entrances.length; i++)
            // {
            //     if(entrances[i][0].toLowerCase() === a.toLowerCase())
            //     {
            //         aux += entrances[i][1] * scaleSegmentRatio;
            //     }
            // }
            aux = entrances[a] !== 'undefined' ? entrances[a] : 0.01;
            aux *= scaleSegmentRatio;
            val = clamp(aux, 0, 8.5);
            ...
            <div class="interaction-all">
              ...
              <div class="viz clearfix">
               ...
               <div class = "metroImage">
                   <a href="https://stackoverflow.com"><img alt="no metro stations image found" src={metroImage}></img></a>
               </div>
            </div>
          </div>
        )
